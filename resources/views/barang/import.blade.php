    ...
    $(document).ready(function() {
        ...
        $("#form-import").validate({
            ...
            submitHandler: function(form) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.status) { // jika sukses
                            ...
                            $('.error-text').text(''); // Kosongkan pesan error
                            $.each(response.msgField, function(prefix, val) {
                                $('#error-' + prefix).text(val[0]); // Tampilkan error pada field terkait
                            });
                            Swal.fire({
                                icon: 'error',
                ...
                return false; // Menghentikan submit form secara langsung
            },
            errorElement: 'span',
            errorPlacement: function(error, element) {
                error.addClass('invalid-feedback');
                element.closest('.form-group').append(error); // Letakkan pesan error di bawah field
            },
            highlight: function(element, errorClass, validClass) {
                $(element).addClass('is-invalid'); // Tambah kelas is-invalid pada field error
            },
            unhighlight: function(element, errorClass, validClass) {
                $(element).removeClass('is-invalid'); // Hapus kelas is-invalid jika tidak ada error
            }
        });
    });
</script>
